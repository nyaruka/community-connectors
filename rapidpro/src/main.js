...
function getConfig(request) {
  ...
  var baseURL = config.newSelectSingle()
    .setId("base_url")
    .setName("Base URL")
    .setIsDynamic(true)
    .setHelpText("The base URL for your RapidPro installation");
           
  baseURL.addOption(config.newOptionBuilder().setLabel("https://app.rapidpro.io - RapidPro").setValue("https://app.rapidpro.io"))
  baseURL.addOption(config.newOptionBuilder().setLabel("https://textit.in - TextIt").setValue("https://textit.in"))
  baseURL.addOption(config.newOptionBuilder().setLabel("Other").setValue("other"))
  ...
  var customURL = config.newTextInput()
    .setId("custom_url")
    .setName("Custom Base URL")
    .setIsDynamic(true)
    .setHelpText("The base URL for your RapidPro endpoint (including http/https)");
  
  // if they selected "other" as theyr base_url, need to ask them what that is
  if (request.configParams.base_url == "other" && request.configParams.custom_url === undefined){
    config.setIsSteppedConfig(true);
    return config.build();
  }

  // now ask for their auth token  
  var apiToken = config.newTextInput()
    .setId("api_token")
    .setName("API Token")
    .setIsDynamic(true)
    .setHelpText("The API Token for your account, find it on your account page");
  ...
  var flows = config.newSelectSingle()
    .setId('flow_uuid')
    .setName("Flow")
  ...
  for (var i=0; i<results.length; i++){
     var flow = results[i];
     flows.addOption(config.newOptionBuilder().setLabel(flow.name).setValue(flow.uuid));
  }
...
function getRecentFlows(config){
  ...
  var response = UrlFetchApp.fetch(getBaseURL(config) + "/api/v2/flows.json?archived=false", options);
...
function getFields(request) {
  ...
  var response = UrlFetchApp.fetch(getBaseURL(request.configParams) + "/api/v2/flows.json?uuid=" + request.configParams.flow_uuid, options);
  var results = JSON.parse(response).results;
  
  var urn = fields.newDimension()
    .setId("_contact_urn")
    .setName("Contact URN")
    .setType(types.TEXT);
...
function fetchPage(configParams, requestedFields, next){
  var url = getBaseURL(configParams) +  "/api/v2/runs.json?flow=" + configParams.flow_uuid;
  if (next != null) {
    url = next;
  }
  var options = {
...
function getData(request) {
  var cc = DataStudioApp.createCommunityConnector();
  var requestedFields = getFields(request).forIds(request.fields.map(function(field) {
      return field.name;
  }));
